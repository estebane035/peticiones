...
(function($) {
  ...
  $('[data-chat-input]').on('keypress',function(e){
    if(e.which == 13 && $.trim($(this).val())!='') {
       var el = $("#my-conversation #vistaChat");
       $(el).append('<div class="message clearfix">'+
        '<div class="chat-bubble from-me">'+$(this).val()+
        '</div></div>');
       mandarMensajeChat($.trim($(this).val()));
       $(this).val('');
       var altura_nueva = $("#vistaChat")[0].scrollHeight;
     $('#vistaChat').scrollTop(altura_nueva);
    }
  });
  $('#addMessage').on('click',function(e){
    if($.trim($("#inputChat").val())!='') {
       var el = $("#my-conversation #vistaChat");
       $(el).append('<div class="message clearfix">'+
        '<div class="chat-bubble from-me">'+$.trim($("#inputChat").val())+
        '</div></div>');
       mandarMensajeChat($.trim($("#inputChat").val()));
       $("#inputChat").val('');
       var altura_nueva = $("#vistaChat")[0].scrollHeight;
     $('#vistaChat').scrollTop(altura_nueva);
...
(function($) {
    ...
    var Quickview = function(element, options) {
        ...
        $(this.options.notes).on('click', '.list > ul > li', function(e) {
            ...
            var note = $(this).find('.note-preview');
            //$("#nota_id").val(note.attr("nota_id")); 
            obtenerNota(note.attr("nota_id"));
            $(_this.options.saveNoteButton).fadeToggle("fast").removeClass('hide');
        ...
        $(this.options.newNoteButton).click(function(e) {
            ...
            $("#nota_id").val('');
            $('#fecha_nota').html(((day<10)?'0'+day:day)+"/"+((month<10)?'0'+month:month)+"/"+year+" "+hours+":"+minutes+":"+seconds);
        });
